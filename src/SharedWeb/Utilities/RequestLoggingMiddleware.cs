...
public sealed class RequestLoggingMiddleware
{
    ...
    public Task Invoke(HttpContext context, IFeatureService featureService)
    {
        using (_logger.BeginScope(
          new RequestLogScope(RequestIdUtilities.GetRequestId(context),
    ...
    private sealed class RequestLogScope : IReadOnlyList<KeyValuePair<string, object?>>
    {
        ...
        public RequestLogScope(string? requestId, string? ipAddress, string? userAgent, string? deviceType, string? origin, string? clientVersion)
        {
            RequestId = requestId;
            IpAddress = ipAddress;
            UserAgent = userAgent;
        ...
        public KeyValuePair<string, object?> this[int index]
        {
            get
            {
                if (index == 0)
                {
                    return new KeyValuePair<string, object?>("requestId", RequestId);
                }
                else if (index == 1)
                {
        ...
        {
            _cachedToString ??= $"requestId:{RequestId} IpAddress:{IpAddress} UserAgent:{UserAgent} DeviceType:{DeviceType} Origin:{Origin} ClientVersion:{ClientVersion}";
